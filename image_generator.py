...
def buscar_imagen_artista(nombre_artista: str, intento: int) -> Image.Image:
    ...
    try:
        ...
        if intento == 0:
            for lang in ["en", "es"]:
                ...
                wiki_response = requests.get(
                    wiki_url, 
                    timeout=10,
                    headers={"User-Agent": "MusicBot/1.0 (ramon@gmail.com)"}
                )
        ...
        # Si no encontramos en Wikipedia, probamos con una búsqueda genérica (Google Images API o similar)
        elif intento == 1:
            url = f"https://api.deezer.com/search/artist?q={nombre_artista.replace(' ', '_')}"
            ...
            #al no estar en otro bucle falla
            ...
            if data.get('data') and len(data['data']) > 0:
                # Intentamos pillar la foto más grande (xl)
                ...
                if img_response.status_code == 200:
                    print(f"✅ Imagen de {nombre_artista} encontrada en Deezer")
                    return Image.open(BytesIO(img_response.content)).convert("RGBA")
        #    search_url = f"https://www.googleapis.com/customsearch/v1?q={nombre_artista.replace(' ', '+')}&cx=YOUR_CSE_ID&key=YOUR_API_KEY&searchType=image&num=1"
        #    search_response = requests.get(search_url, timeout=10)
        #    if search_response.status_code == 200:
        #        search_data = search_response.json()
        #        if "items" in search_data and len(search_data["items"]) > 0:
        #            img_url = search_data["items"][0]["link"]
        #            img_response = requests.get(img_url, timeout=10,
        #                                        headers={"User-Agent": "MusicBot/1.0"})
        #            return Image.open(BytesIO(img_response.content)).convert("RGBA")
...
def generar_portada(plantilla_olas_path: str, plantilla_path: str, titulo: str, subtitulo: str,
    ...
    for intento in range(3):
        foto = buscar_imagen_artista(nombre_artista, intento)
        if foto is not None:
            print(f"✅ Imagen encontrada en intento {intento + 1}")
            break
        print(f"⚠️ Intento {intento + 1} fallido, probando siguiente fuente...")

    if foto is None:
        print("⚠️ No se encontró imagen del artista, usando plantilla sin foto")
        # Continuamos sin foto en vez de parar el bot
...
def generar_portada_tiktok(plantilla_olas_path: str, plantilla_path: str, titulo: str, subtitulo: str,
                    nombre_artista: str, output_path: str):
    """Genera la portada final con foto, título y subtítulo"""
    ...
    if foto is None:
        print("⚠️ No se encontró imagen del artista, usando plantilla sin foto")
        foto_sin_fondo = Image.open("assets/foto_sin_fondo.png")
        # Continuamos sin foto en vez de parar el bot
    else:
        session = new_session("isnet-general-use")
        foto_sin_fondo = remove(
